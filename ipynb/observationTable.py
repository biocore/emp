...
logfiles = get_ipython().getoutput('find "$path" -name "$filename"')
...
for logfile in logfiles:
    ...
    get_ipython().system('grep "^$studyID" "$logfile" > "$tmpfile"')
...
if fasta['split-libraries']['label'] in observations:
    ...
    tmpfile = "/tmp/sampleIDs"
    filenames_filterSeqs = get_ipython().getoutput('find "$path" -name "$filename"')
    pdsFiltered = []
    print("Step 3/4: parsing " + str(len(filenames_filterSeqs)) + " '" + filename + "' files: ", end="")
    for file in filenames_filterSeqs:
        print(".", end="")
        studyID = logfile.split("/")[-2]
        get_ipython().system('grep "^>" "$file" | cut -d "_" -f 1 | sort | tr -d ">" | uniq -c | sed "s/^[ \\t]*//" > "$tmpfile" ')
